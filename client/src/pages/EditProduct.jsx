...
const EditProduct = () => {
  ...
  useEffect(() => {
    if (product?.product) {
      ...
      // Convert existing images to full URLs for display
      const existingImages = (productData.images || []).map(img => getImageUrl(img));
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      // Create temporary URLs for preview of new images
  ...
  const onSubmit = (data) => {
    ...
    // Convert full URLs back to relative paths for existing images
    const processedImages = images.map(img => {
      // Handle any server URL (localhost or network IP)
