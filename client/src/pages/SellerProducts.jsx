...
const SellerProducts = () => {
  ...
  const { data: productsData, isLoading: productsLoading } = useQuery({
    queryKey: ['seller-products', user?.id, searchTerm, selectedCategory, currentPage],
    enabled: !!user?.id,
    queryFn: async () => {
  ...
  const deleteProductMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['seller-products', user?.id]);
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
              {productsData.products.map((product) => (
                <div key={product.id || product._id} className="card p-4">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Link
                      href={`/seller/products/edit/${product.id || product._id}`}
