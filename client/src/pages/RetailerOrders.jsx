...
const RetailerOrders = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['retailer-orders-page', user?.id],
    enabled: !!user?.id,
          ...
          <div className="space-y-6">
            {orders.map(order => (
              <div key={order.id || order._id} className="card p-6">
                ...
                {(order.status === 'shipped' || order.status === 'out_for_delivery' || order.status === 'delivered') && order.invoicePath && (
                  <div className="mt-4">
                    <button 
                      onClick={() => handleInvoiceDownload(order.id || order._id)}
                ...
                {order.status === 'ordered' && (
                  <div className="mt-4 text-sm text-text-secondary">
                    Invoice will be available once the order is shipped.
                  </div>
                )}
              </div>
            ))}
